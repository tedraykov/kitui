<script lang="ts">
	...
	import type { ComponentControl, PropDefinition, PropDefinitionsMap, Props } from '../../types';
	...
	const defaultColumns: ColumnDef<PropDefinition>[] = [
		...
		},
		{
			...
			cell: (info) => renderComponent(HtmlCell, { html: info.getValue<string>() }),
			header: () => 'Description'
		},
		{
			accessorKey: 'default',
			header: () => 'Default Value',
			cell: (info) => renderComponent(CodeCell, { text: info.getValue<Props[keyof Props]>() })
		},
		{
			accessorKey: 'control',
			header: () => 'Control',
			cell: (info) => {
				...
				const { type, props } = info.getValue<ComponentControl>();
